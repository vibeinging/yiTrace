//! 规模压测入口：用真实 durable engine 写入，再通过 `EngineJsonApi` 读模型 API 查询。
//!
//! 目标不是替代微基准，而是给“这个版本在当前机器能扛多大”一个可归档报告。
//! 跑法：
//!   cargo run --release -p yt-engine --example scale_bench -- --spans 100000 --queries 500
...
const DEFAULT_VECTOR_QUERIES: usize = 5;
const DEFAULT_DIM: usize = 32;
const DEFAULT_VECTOR_STRIDE: usize = 10;
const TENANT: u64 = 42;
...
fn main() {
    ...
    let vector_count = args.spans / args.vector_stride.max(1);
    eprintln!(
        "scale_bench: vector index count={} dim={} stride={}",
        vector_count, args.dim, args.vector_stride
    );
    let vector_start = Instant::now();
    for i in (1..=args.spans).step_by(args.vector_stride.max(1)) {
        let body = vector_index_body(i as u64, &mut rng, args.dim);
        let (status, resp) = api.route_with_tenant("POST", "/v1/vector-index", &body, Some(TENANT));
        if status != 200 {
            panic!("vector-index failed: status={status}, body={resp}");
        }
    }
    let vector_index_duration = vector_start.elapsed();

    let queries = build_queries(args.queries, args.vector_queries, args.dim);
...
impl Args {
    fn parse() -> Self {
        ...
        let mut vector_queries = DEFAULT_VECTOR_QUERIES;
        let mut dim = DEFAULT_DIM;
        let mut batch = 512usize;
        let mut vector_stride = DEFAULT_VECTOR_STRIDE;
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--vector-queries" => vector_queries = parse_next(&mut args, "--vector-queries"),
                "--dim" => dim = parse_next(&mut args, "--dim"),
                "--batch" => batch = parse_next(&mut args, "--batch"),
                "--vector-stride" => vector_stride = parse_next(&mut args, "--vector-stride"),
        ...
        Self {
            ...
            dim,
            batch,
            vector_stride: vector_stride.max(1),
            data_dir,
            report,
            cold_queries,
            keep_data,
...
fn print_help() {
    eprintln!(
        "usage: scale_bench [--spans N] [--queries N] [--vector-queries N] [--dim N] [--batch N] [--vector-stride N] [--data-dir DIR] [--report PATH] [--cold-queries] [--keep-data]"
    );
}
...
fn make_record(id: u64, tenant: u64, rng: &mut Rng) -> WireRecord {
    ...
    WireRecord {
        ...
        cached_input_tokens: Some(rng.next() % 50),
        reasoning_tokens: Some(rng.next() % 80),
        total_tokens: None,
        cost_usd_nanos: None,
        cost_currency: None,
...
fn vector_index_body(trace_id: u64, rng: &mut Rng, dim: usize) -> String {
    let vector = rng
        .vec(dim)
        .into_iter()
        .map(|v| format!("{v:.4}"))
        ...
        .join(",");
    format!(
        r#"{{"namespace":"task","key":"task-{trace_id}","traceId":{trace_id},"vector":[{vector}],"attrs":{{"project_id":"scale-a","task_fingerprint":"risk-review"}}}}"#
    )
}
...
fn build_queries(count: usize, vector_count: usize, dim: usize) -> Vec<Query> {
    let vector = (0..dim)
        .map(|i| format!("{:.4}", (i % 17) as f32 / 17.0))
        .collect::<Vec<_>>()
        .join(",");
    vec![
        Query {
        ...
        Query {
            ...
            body: format!(
                r#"{{"namespace":"task","vector":[{vector}],"k":10,"filter":{{"attrs":{{"project_id":"scale-a"}}}}}}"#
            ),
        },
        Query {
            name: "trace_search_page_100",
            count,
            method: "POST",
            path: "/v1/trace-search",
            body: format!(r#"{{"filter":{{"projectId":"scale-a"}},"limit":{} }}"#, count.min(100)),
        },
    ]
}
...
fn render_report(report: &Report) -> String {
    ...
    let vector_qps =
        report.vector_count as f64 / report.vector_index_duration.as_secs_f64().max(0.001);
    ...
    let _ = writeln!(out, "- vectorQueries: {}", report.vector_queries);
    let _ = writeln!(
        out,
        "- queryCacheMode: {}",
        if report.cold_queries { "cold" } else { "warm" }
    );
    let _ = writeln!(out, "- vectorDim: {}", report.dim);
    let _ = writeln!(out, "- vectorCount: {}", report.vector_count);
    ...
    let _ = writeln!(
        out,
        "| vector_index | {} | {:.3} | {:.0} vectors/s |",
        report.vector_count,
        report.vector_index_duration.as_secs_f64(),
        vector_qps
    );
    let _ = writeln!(out);
