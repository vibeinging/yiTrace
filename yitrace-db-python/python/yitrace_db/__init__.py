...
class YiTraceDB:
    ...
    def vector_index(self, body: Mapping[str, Any], *, tenant_id: TenantId | None = None) -> Any:
        return self.route_json("POST", "/v1/vector-index", body, tenant_id=tenant_id)

    def vector_search(self, query: Mapping[str, Any], *, tenant_id: TenantId | None = None) -> Any:
        return self.route_json("POST", "/v1/vector-search", query, tenant_id=tenant_id)

    index_vector = vector_index
    search_vector = vector_search
    traceSearch = trace_search
    traceAggregate = trace_aggregate
    indexVector = vector_index
    searchVector = vector_search

    def traces(self, *, tenant_id: TenantId | None = None, **options: Any) -> Any:
        attrs = _normalize_attrs(options)
        query = _query_string({"attrs": attrs, **options})
        return self.route_json("GET", _append_query("/v1/traces", query), tenant_id=tenant_id)
    ...
    def spans(self, trace_id: TenantId, cursor: int = 0, limit: int = 50, include_full: bool = False, *, tenant_id: TenantId | None = None) -> Any:
        trace_part = quote(str(trace_id), safe="")
        path = f"/v1/traces/{trace_part}/spans?cursor={cursor}&limit={limit}&includeFull={1 if include_full else 0}"
        return self.route_json("GET", path, tenant_id=tenant_id)

    def annotate(self, annotation: Mapping[str, Any], *, tenant_id: TenantId | None = None) -> Any:
        return self.route_json("POST", "/v1/annotations", annotation, tenant_id=tenant_id)

    def annotations(self, *, tenant_id: TenantId | None = None, **filter: Any) -> Any:
        return self.route_json("GET", _append_query("/v1/annotations", _query_string(filter)), tenant_id=tenant_id)

    def link_dataset_item(self, association: Mapping[str, Any], *, tenant_id: TenantId | None = None) -> Any:
        return self.route_json("POST", "/v1/dataset-associations", association, tenant_id=tenant_id)
    ...
    def create_golden_path(self, candidate: Mapping[str, Any], *, tenant_id: TenantId | None = None) -> Any:
        return self.route_json("POST", "/v1/golden-paths", candidate, tenant_id=tenant_id)

    createGoldenPath = create_golden_path

    def update_golden_path_status(self, golden_path_id: TenantId, update: Mapping[str, Any] | str, *, tenant_id: TenantId | None = None) -> Any:
        body = {"status": update} if isinstance(update, str) else dict(update)
        path = f"/v1/golden-paths/{quote(str(golden_path_id), safe='')}/status"
        return self.route_json("POST", path, body, tenant_id=tenant_id)

    updateGoldenPathStatus = update_golden_path_status

    def path_adherence(self, golden_path_id: TenantId, trace_id: TenantId, *, tenant_id: TenantId | None = None) -> Any:
        return self.route_json("POST", "/v1/path-adherence", {"goldenPathId": golden_path_id, "traceId": trace_id}, tenant_id=tenant_id)

    pathAdherence = path_adherence
