# yitrace trace-sdk (Python)

Python trace SDK for yiTrace. Use this when you want to record agent runs
...
> License: MIT. Requires Python >= 3.8. Runtime dependencies: none.
...
全都对得上（同一 span 重传/崩溃恢复不会被算两遍，token/费用不翻倍）。这让 `trace-sdk`
可以安全采用 at-least-once 上报。
...
tr.close()  # flush -> POST to yiTrace
```
