...
> 一台**单机、单目录、零外部依赖**的 AI Agent 可观测性数据库引擎。用 Rust 自研，把 Agent（多轮对话、调工具、多 agent 协作）跑出来的 trace 灌进来，提供 trace 还原、中文 BM25 检索、带过滤的向量召回、成本归因和评测闭环。

**English** · [中文（本文件）](README.zh-CN.md)
...
# 起 HTTP 摄入/查询服务（8 线程池，自带 eval 种子数据）
cargo run -p yt-engine --example server
```

服务起来后（`http://127.0.0.1:7878`）：
