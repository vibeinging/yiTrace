...
- **嵌入式 DB 包**：`@yitrace/db`、Python `yitrace-db`、Rust `yitrace-db` 都通过进程内 `EngineJsonApi` 调引擎，不直接解析 WAL / manifest / segment 文件。embedded 模式只适合单进程持有一个 writer；多 worker / 多容器共享同一 data dir 时，起一个 yiTrace server 或单 writer 进程。
