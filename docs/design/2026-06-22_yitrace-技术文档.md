...
> 现状：alpha。Rust 引擎主体刻意只用标准库、零外部依赖，`cargo test --offline` 离线可过。Node / Python / Rust 嵌入式 DB、HTTP API、控制台、中文检索、向量召回、attrs 过滤、rollup、metadata/retention 基座已打通。生产安全和百万 span 上量优化仍在路线图中。

---

## 1. 它是什么

一台**本地优先、单目录落盘、可作为 server 也可嵌入应用进程**的 AI Agent 可观测性数据库引擎。Agent（多轮对话、调工具、多 agent 协作）跑起来的 trace（谁调了谁、花了多少 token、慢在哪、改 prompt 后变好还是变差）灌进来，引擎把它存成审计级不重不漏的数据，并提供中文检索 + 带过滤语义召回 + 评测三类查询。
...
3. **本地私有化**：一个目录装下全部状态，可以独立起 HTTP server，也可以被 Node / Electron / Python / Rust 应用嵌入，数据不出境。
...
1. **中文分词** = 纯 Rust 词级分词 `ChineseTokenizer`。它内嵌 jieba 全量词典，走词典 DAG + 最大概率 DP；ASCII/数字按空白与标点切词、小写化。`with_user_dict` 可以叠加业务词。
2. **BM25 打分**：真倒排（token → 每文档词频）+ idf + 文档长度归一（K1=1.5, B=0.75）。按相关性排序，不是子串「有/无」。
3. **WAND 剪枝**：内存 BM25 已有 block-max-WAND 风格剪枝，并用测试保证与暴力打分一致。百万级冷启动和低内存场景还要补段内持久倒排、分块 postings 和段内 block-max-WAND。

为什么比子串强：查「盗刷风控」这种**非连续多概念**串，子串占位要求文档里出现连续「盗刷风控」才命中 → 一条都召不回；BM25 会按词级 token 算相关性，命中两类概念的文档排在前面。模块自带一个**会失败的测试**钉住这个差距。

### 7.2 图式向量 ANN + 带过滤召回（`graph.rs`）

这块验证整个自研路线里红队点名的最大翻车点：**带过滤的近邻搜索能不能把召回拉回来**。当前默认实现是 `DiskGraphIndex`：磁盘型多层 HNSW，上层稀疏图常驻内存，底层节点和向量落盘，向量读取走字节预算缓冲池。重点仍是两种过滤策略对比：
...
模块自带会失败的测试：选择性谓词下实测 **in-graph 召回 ≫ post-filter 召回**。`DiskGraphIndex` 支持重启后从磁盘恢复，不需要每次 rebuild；后续上量方向是 SIMD、量化（SQ/PQ）和更正式的大规模 recall/latency 基线。

### 7.3 混合检索与属性过滤

- `search_hybrid*`：关键词 BM25 + 语义 ANN 两路用 **RRF（Reciprocal Rank Fusion）** 融合，同时命中的排更前。
- `search_*_attr`：在向量/文本召回之上叠 `SearchFilter`（按 agent / 状态 / 时间 / attrs 过滤），实现「找 agent『风控研判』报错的相似 span」这类按业务维度的语义召回。过滤条件下推进图搜索（§7.2），不是事后筛。
...
引擎主体刻意零依赖，但存储、分词、向量索引都留了 trait 接缝。默认路径已经可用；外部 crate 是隔离重依赖、客户适配和对标用，不是主线能力缺口。

| trait | 默认实现 | 可选外部适配 |
|---|---|---|
| `SegmentStore` | `FileSegmentStore`（std-only 行式段） | `yitrace-segstore-vortex`：Vortex 列式段，谓词下推 + 投影下推 |
| `Tokenizer` / `Bm25Index` | `ChineseTokenizer` + BM25 倒排（纯 Rust，内嵌 jieba 全量词典） | `yitrace-tokenizer-jieba`：可选 cppjieba FFI |
| `GraphIndex` | `DiskGraphIndex`（磁盘型多层 HNSW，缓冲池，进图过滤） | `yitrace-vecindex-graph`：可选 graph_index FFI |

Vortex、cppjieba、graph_index 相关代码都**刻意建在引擎工作区之外**：重依赖或 FFI 只在外部 crate 里，引擎主体保持 std-only、离线可测试。落地计划见 [`docs/design/2026-06-22_列式段存储-vortex-选型与落地计划.md`]。
...
- **SDK / client facade**：Python `yitrace` 提供 `connect(url=...)` 连接远程 yiTrace server，`connect(path=...)` 在安装 `yitrace-db` 后打开本地 embedded DB；`DbExporter` 让同一个 `Tracer` 可以直接写本地 DB。Rust `yitrace` crate 是纯 std 打点 SDK，负责 `Tracer`、`HttpExporter` 和确定性 event_id。
- **嵌入式 DB 包**：`@yitrace/db`、Python `yitrace-db`、Rust `yitrace-db` 都通过进程内 `EngineJsonApi` 调引擎，不直接解析 WAL / manifest / segment 文件。embedded 模式支持同机多个进程打开同一个本地 data dir；引擎内部用 data-dir 锁串行化 open/write，并用 reader pin 保护跨进程快照回收。多机器、网络文件系统或跨主机容器共享同一 data dir 时，起一个 yiTrace server。
- **Python server mode**：`yitrace-db` 提供可选 FastAPI router 和 `yitrace-db serve` CLI。它是把一个 embedded DB handle 暴露成 HTTP 服务，不改变底层单写者约束。
...
- **极小 HTTP/1.1 服务**（`http/`，只用 `std::net`）：`POST /v1/ingest`、`POST /v1/traces`（OTLP）、`GET /v1/traces`（列表）、`POST /v1/search`（检索），以及控制台、读模型、metadata、retention 相关 `/v1/*` 端点。支持 `Authorization: Bearer <token>` 鉴权（None = 仅本机开发）。上量/上 TLS 时换 axum/hyper，路由逻辑不变。
...
- **构建**：`cd yitrace-engine && cargo test --offline`（离线可过，零外部依赖）。Vortex / N-API / PyO3 等重依赖都在工作区外单独构建。
- **包形态回归**：从仓库根目录跑 `./scripts/package_mode_eval.sh`，覆盖 Python SDK、TypeScript SDK、Node embedded DB、Python embedded DB 和 Rust embedded crate。
- **测试文化**：多数测试是**带会失败的不变量测试**，用来证明技术前提成立：中文多概念召回、in-graph 召回优于 post-filter、崩溃重放幂等、flush 后重启不丢、并发快照隔离、attrs index/rollup 命中与 fallback、metadata/retention 持久化、package-mode ESM/CJS/native/Python/Rust 入口等。

### 现状与诚实边界

| 已用代码验证 | 还没做（上量必需） |
|---|---|
| 确定性 event_id 三方一致、重传/重放幂等 | TLS、RBAC、落盘加密、限流、持久审计日志 |
| 单写者 + 四源折叠归并 + 快照 pin 协议 | 百万 span 冷/热缓存规模压测 |
| 纯 Rust 词级中文 BM25、in-graph 带过滤召回 | BM25 段内持久倒排 + 分块 postings |
| WAL/段/manifest/向量四件落盘、重启不丢 | attrs postings 磁盘分页 buffer manager |
| Node/Python/Rust embedded DB + HTTP API 共用 `EngineJsonApi` | loop/task/trajectory 独立磁盘物化索引 |
| metadata / retention / rollup / attrs sidecar 主线版 | LLM-judge eval、DataFusion 查询执行 |

一句话：**核心技术前提已经用代码验证通过，当前重点不是推倒重来，而是把生产边界和规模化索引补齐。**
